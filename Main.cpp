...
int binary_search(const vector<T> &items, int first, int last, T target) {
	...
	else if (items[pivot] > target) { //Modifies funciton 

		binary_search(items, first, pivot - 1, target); //Function Calling itself

	}
	else if (items[pivot] == target) { //Base case

		return pivot;
	}
	else if (items[pivot] < target) { //Modifies function

		binary_search(items, pivot + 1, last, target); //Function calling itself
	}
